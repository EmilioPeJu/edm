...
static void logMeterc_edit_update (
  Widget w,
  XtPointer client,
  XtPointer call )
{
...
static void logMeterc_edit_apply (
  Widget w,
  XtPointer client,
  XtPointer call )
{
  ...
  logMeterc_edit_update ( w, client, call );
  metero->refresh ( metero );

}

static void logMeterc_edit_ok (
  Widget w,
  XtPointer client,
  XtPointer call )
{
...
static void logMeterc_edit_cancel (
  Widget w,
  XtPointer client,
  XtPointer call )
{
...
static void logMeterc_edit_cancel_delete (
  Widget w,
  XtPointer client,
  XtPointer call )
{
...
static void logMeter_monitor_read_connect_state (
  ProcessVariable *pv,
  void *userarg )
{
...
static void logMeter_readUpdate (
  ProcessVariable *pv,
  void *userarg )
{
...
activeLogMeterClass::activeLogMeterClass ( void ) 
{
  ...
  strcpy ( name, "activeLogMeterClass" );
  minW = 100;
  minH = 50;
  strcpy ( scaleFontTag, "" );
  strcpy ( labelFontTag, "" );
...
int activeLogMeterClass::createInteractive (
...
int activeLogMeterClass::old_save (
  FILE *f )
{
...
int activeLogMeterClass::createFromFile (
  FILE *f,
  char *name,
...
int activeLogMeterClass::old_createFromFile (
  FILE *f,
  char *name,
  activeWindowClass *_actWin )
{
...
int activeLogMeterClass::genericEdit ( void ) 
{
  ...
  ptr = actWin->obj.getNameFromClass ( "activeLogMeterClass" );
  if ( ptr )
    strncpy ( title, ptr, 31 );
  else
    strncpy ( title, activeLogMeterClass_str2, 31 );

  Strncat ( title, activeLogMeterClass_str3, 31 );
  ...
  ef.addTextField ( activeLogMeterClass_str4, 35, &eBuf->bufX );
  ef.addTextField ( activeLogMeterClass_str5, 35, &eBuf->bufY );
  ef.addTextField ( activeLogMeterClass_str6, 35, &eBuf->bufW );
  ef.addTextField ( activeLogMeterClass_str7, 35, &eBuf->bufH );
  //   ef.addTextField ( activeLogMeterClass_str8, 35, eBuf->bufControlPvName,
  //    PV_Factory::MAX_PV_NAME );
  ...
  ef.addOption ( activeLogMeterClass_str17, activeLogMeterClass_str43,
   eBuf->bufScaleFormat, 15 );
  //ef.addTextField ( activeLogMeterClass_str19, 35, &eBuf->bufScalePrecision );
  ef.addTextField (activeLogMeterClass_str19, 35, eBuf->bufScalePrecision, 15 );
  ef.addToggle ( activeLogMeterClass_str20, &eBuf->bufScaleLimitsFromDb );
  //ef.addTextField (activeLogMeterClass_str21, 35, &eBuf->bufScaleMin);
...
int activeLogMeterClass::draw ( void ) 
{
  ...
  double needlePlusScale, scaleLogTextIncr, scaleLogValue;
  double scaleLogMax, scaleLogMin;
...
//}
  ...
  scaleLogTextIncr = (scaleLogMax - scaleLogMin) / labelIntervals;
  ...
  if ((labelIntervals != 0) && (labelIntervals !=0 && showScale))
  {
    ...
    for (labelAngle = beginAngle, scaleLogValue = scaleLogMax;
         ...
         labelAngle += labelAngleIncr)
    {
      for (i = 0; i < majorIntervals; i++)
      {
        for (ii = 1; ii < minorIntervals; ii++)
        {
          ...
          nearEndY = (int) (meterNeedleYorigin -
                           insideArc * sin (ii * minorAngleIncr + labelAngle +
                                            i * majorAngleIncr));
          XDrawLine (actWin->d, XtWindow (actWin->drawWidget),
                     actWin->drawGc.normGC (), farEndX, farEndY,
                     nearEndX, nearEndY );
        }
        ...
        if ( i > 0 )
        {
          ...
          farEndY = (int) (meterNeedleYorigin -
                           (insideArc + tickSize) *
                           sin (i * majorAngleIncr + labelAngle));
          nearEndX = (int) (meterNeedleXorigin +
...
//}
  ...
  if (readV > 0)
    needleAngle = beginAngle +
                  meterTotalAngle * (1.0 - ((log10 (readV) - scaleLogMin) /
...
int activeLogMeterClass::activate ( int pass, void *ptr )
{
  ...
  case 2:
    if ( !opComplete )
    {
      ...
      if ( readExists ) 
      {
        ...
        {
          fprintf ( stderr, activeLogMeterClass_str38 );
          opStat = 0;
        }
      }
...
void activeLogMeterClass::updateDimensions ( void )
{
  if ( scaleFs )
  {
...
void activeLogMeterClass::btnUp (
  int x,
  int y,
  int meterState,
  int meterNumber )
...
void activeLogMeterClass::btnDown (
  int x,
  int y,
  int meterState,
  int meterNumber )
...
void activeLogMeterClass::btnDrag (
  int x,
  int y,
  int meterState,
  int meterNumber )
...
int activeLogMeterClass::getMeterActionRequest (
  int *up,
  int *down,
  int *drag )
{
...
void activeLogMeterClass::bufInvalidate ( void )
{
  drawStaticFlag = 1;
  bufInvalid = 1;
...
int activeLogMeterClass::expand1st (
  int numMacros,
  char *macros[],
  char *expansions[] )
{
...
int activeLogMeterClass::expand2nd (
  int numMacros,
  char *macros[],
  char *expansions[] )
{
...
int activeLogMeterClass::checkResizeSelectBox (
  int _x,
  int _y,
  int _w,
  int _h ) 
...
int activeLogMeterClass::checkResizeSelectBoxAbs (
  int _x,
  int _y,
  int _w,
  int _h )
...
void activeLogMeterClass::executeDeferred ( void ) 
{
  ...
  if ( ni )
  {
    if ( initialReadConnection )
    {
      ...
      readPvId->add_value_callback ( logMeter_readUpdate, this );
...
char *activeLogMeterClass::dragValue ( int i )
{
...
void activeLogMeterClass::changeDisplayParams (
  unsigned int _flag,
  char *_fontTag,
  int _alignment,
  char *_ctlFontTag,
...
void activeLogMeterClass::changePvNames (
  int flag,
  int numCtlPvs,
  char *ctlPvs[],
  int numReadbackPvs,
...
void activeLogMeterClass::getPvs (
  int max,
  ProcessVariable *pvs[],
  int *n )
{
...
char *activeLogMeterClass::crawlerGetFirstPv ( void )
{
  crawlerPvIndex = 0;
  return readPvExpStr.getExpanded ();
}
...
void *create_activeLogMeterClassPtr ( void )
{
  activeLogMeterClass *ptr;
  ptr = new activeLogMeterClass;
  return (void *) ptr;
}

void *clone_activeLogMeterClassPtr ( void *_srcPtr )
{
  activeLogMeterClass *ptr, *srcPtr;
  srcPtr = (activeLogMeterClass *) _srcPtr;
  ptr = new activeLogMeterClass ( srcPtr );
  return (void *) ptr;
}
